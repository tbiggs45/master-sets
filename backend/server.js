...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && pathname === "/feedback") {
    try {
      ...
      if (!text) { sendJson(req, res, 400, { error: "Feedback text is required" }); return; }
      if (!FEEDBACK_TO || !SMTP_USER || !SMTP_PASS) {
        sendJson(req, res, 200, { ok: true }); return; // not configured — accept silently
      }
      const transport = buildMailTransport();
      await transport.sendMail({ from: SMTP_USER, to: FEEDBACK_TO, subject: "Master Sets — App Feedback", text });
      sendJson(req, res, 200, { ok: true });
    } catch (e) {
      console.error("Feedback send error:", e.message);
      sendJson(req, res, 500, { error: "Could not send feedback" });
    }
    return;
  }
